...
open class TimestampRulesFilter: LogitsFiltering {
  ...
  public func filterLogits(_ logits: MLMultiArray, withTokens tokens: [Int]) -> MLMultiArray {
    ...
    logits.fill(
      indexes: [[0, 0, specialTokens.noTimestampsToken as NSNumber]],
      with: FloatType.negativeInfinity)

    if tokens.count > sampleBegin {
      // timestamps have to appear in pairs, except directly before EOT; mask logits accordingly
      let sampledTokens = tokens[sampleBegin...]
      let lastWasTimestamp =
        sampledTokens.count >= 1 && sampledTokens.last! >= specialTokens.timeTokenBegin
      let penultimateWasTimestamp =
        sampledTokens.count < 2 || sampledTokens.dropLast().last! >= specialTokens.timeTokenBegin
      if lastWasTimestamp {
        if penultimateWasTimestamp {
          // has to be non-timestamp
          logits.fillLastDimension(
            indexes: specialTokens.timeTokenBegin..<logits.count, with: FloatType.negativeInfinity)
        } else {
          // cannot be normal text tokens
          logits.fillLastDimension(
            indexes: 0..<specialTokens.endToken, with: FloatType.negativeInfinity)
        }
      }
      ...
      if let lastTimestamp = timestamps.last {
        ...
        logits.fillLastDimension(
          indexes: specialTokens.timeTokenBegin..<timestampLast, with: FloatType.negativeInfinity)
      }
    }
    ...
    if sumOfProbabilityOverTimestampsIsAboveAnyOtherToken(
      logits: logits, timeTokenBegin: specialTokens.timeTokenBegin)
    {
      logits.fillLastDimension(
        indexes: 0..<specialTokens.timeTokenBegin, with: FloatType.negativeInfinity)
    }
    return logits
  }
  ...
  private func sampleBegin(for tokens: [Int]) -> Int? {
    if isModelMultilingual {
      ...
      if let taskTokenIndex = tokens.prefix(3).firstIndex(where: {
        $0 == specialTokens.transcribeToken || $0 == specialTokens.translateToken
      }) {
...
open class LanguageLogitsFilter: LogitsFiltering {
  ...
  public init(allLanguageTokens: Set<Int>, logitsDim: Int, sampleBegin: Int) {
    ...
    self.sampleBegin = sampleBegin
    self.nonLanguageTokenIndexes = LanguageLogitsFilter.getNonLanguageTokenIndexes(
      logitsDim: self.logitsDim, allLanguageTokens: self.allLanguageTokens)
  }
  ...
  private static func getNonLanguageTokenIndexes(logitsDim: Int, allLanguageTokens: Set<Int>)
    -> [[NSNumber]]
  {
...
extension UInt16 {
  public static let infinity: UInt16 = 0x7C00  // Float16 positive infinity
  public static let negativeInfinity: UInt16 = 0xFC00  // Float16 negative infinity
}
